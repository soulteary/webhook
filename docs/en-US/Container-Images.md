...
WebHook 7.3.1 publishes three multi-architecture runtime variants to Docker Hub
...
|---|---|---|---|
| Core | `core-7.3.1` | `scratch`, CA certificates, WebHook | Every executed command is a mounted static binary |
| Runner | `runner-7.3.1` | Alpine, BusyBox utilities, Bash, CA certificates | Hooks execute ordinary POSIX shell or Bash scripts |
| Extended | `extended-7.3.1` | Runner plus curl, jq, and yq | Hooks explicitly require network or JSON/YAML command-line tools |

The unprefixed `7.3.1` and `latest` tags are aliases for `core-7.3.1`. Pin a
...
The old `extend-7.3.1` tag remains as a compatibility alias for
`extended-7.3.1`. Existing deployments can move without an immediate rename,
...
- Move shell-only hooks to `runner-7.3.1`.
- Keep hooks that require curl, jq, or yq on `extended-7.3.1`.
- Use `core-7.3.1` only when the configured command is a static executable.
