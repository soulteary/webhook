...
| `-hooks-dir string` | Directory to scan for hook config files (*.json, *.yaml, *.yml); if empty, watch for new files; use with Config UI to enable save-to-dir | - |
...
Mode is determined by whether `-hooks` is set and whether `-config-ui` is enabled:

- **Config UI only:** When `-config-ui` is set and `-hooks` is not (neither CLI nor env), only the Config UI HTTP server runs (default port 9080).
- **Webhook + Config UI:** When both `-hooks` and `-config-ui` are set, Config UI is mounted on the webhook server (e.g. `http://localhost:9000/config-ui`).
- **Webhook only:** When `-hooks` is set and `-config-ui` is not, only the webhook service runs.

| Flag | Description | Default |
|------|-------------|---------|
| `-config-ui` | Enable config generator Web UI; without `-hooks` runs config-ui-only mode (default port 9080); recommend only for debugging or intranet | `false` |
| `-config-ui-path string` | HTTP path for Config UI when mounted on webhook server (trailing slash is normalized; e.g. `/config-ui/` becomes `/config-ui`) | `/config-ui` |

*Note:* In config-ui-only mode (no `-hooks`), the UI is served at `/`. When Config UI is mounted on the webhook server, it uses `-config-ui-path`. Do not set `-config-ui-path` to a reserved path (e.g. `/`, `/health`, `/hooks`, `/openapi`); if you do, the Config UI route will not be registered and a warning will be logged. The call URL and curl example generated by the Config UI use `-urlprefix` (e.g. with `-urlprefix=events`, the generated URL is `/events/:id`). With `-hooks-dir` set, the UI exposes a "Save to directory" option.
...
| `PORT` | `-port` | Listen port; defaults to `9080` in config-ui-only mode (no `HOOKS` and Config UI enabled), otherwise `9000` | `9000` or `9080` |
| `HOOKS` | `-hooks` | Hook file paths (comma-separated) | - |
| `HOOKS_DIR` | `-hooks-dir` | Directory to scan for hook config files; enables save-to-dir in Config UI when set | - |
| `URL_PREFIX` | `-urlprefix` | URL prefix for hooks and Config UI generated call URL | `hooks` |
...
| Environment Variable | CLI Flag | Description | Default |
|---------------------|----------|-------------|---------|
| `CONFIG_UI_ENABLED` | `-config-ui` | Enable config generator Web UI; without `HOOKS` runs config-ui-only mode (default port 9080) | `false` |
| `CONFIG_UI_PATH` | `-config-ui-path` | HTTP path for Config UI when mounted on webhook server | `/config-ui` |
